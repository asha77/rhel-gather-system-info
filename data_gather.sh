#!/bin/bash
#
# Modified Gather and package a select set of information about a RHEL or CentOS 6, 7 or 8 system.
# ============================
#
# USAGE
...
GLOBAL_SCRIPT_VERSION="0.9"
GLOBAL_SCRIPT_BUILD_ID="20230706"
...
other_files="/var/log:dmesg"
commands=("ifconfig -a"
"ip route"
"ip link"
"ip address"
"ping 10.128.255.201 -c 4"
"ping 10.255.114.197 -c 4"
"ping 10.255.114.198 -c 4"
"ping 10.255.113.65 -c 4"
"ping 10.255.113.1 -c 4"
"ping 10.128.255.238 -c 4"
"ping 195.239.218.46 -c 4"
"ping 10.12.33.78 -c 4"
"ping 10.12.34.206 -c 4"
"ping 10.128.255.231 -c 4"
"ping 10.128.255.227 -c 4"
"uname -a"
"df -hT"
"cat /etc/os-release"
...
check_os() {
	if [[ -r "/etc/redhat-release" ]]; then
		...
		if [[ "${os_version}" -lt 6 || "${os_version}" -gt 9 || ! ( "$(echo "${os_full}" | grep -oi "centos" | wc -l)" -gt 0 || "$(echo "${os_full}" | grep -oi "red hat enterprise linux" | wc -l)" -gt 0 ) ]]; then
			echo -e "\n[ERROR]: Operating system not supported. Supported operating systems: RHEL and CentOS 6, 7 and 8.\n"
#			exit 125
		fi
	else
                os_full="$(cat /etc/os-release)"
                os_version="$(echo "${os_full}" | grep -oE '[0-9]+\.[0-9]+' | head -1 | cut -d'.' -f1)"
